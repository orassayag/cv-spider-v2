...
Built in February 2013. This is the second version of CV Spider that searches with Walla search (which uses Google search in its view source) and Bing search engine for job listings, extracts email addresses that match specific search patterns, and stores them in a SQL Server database for automated outreach.
